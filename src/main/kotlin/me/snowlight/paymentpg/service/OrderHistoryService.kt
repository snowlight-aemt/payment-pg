...
import me.snowlight.paymentpg.config.common.query
...
class OrderHistoryService(
    ...
    suspend fun getHistory(request: QryOrderHistory): List<Order> {
        val param = HashMap<String, Any>().apply {
            put("userId", request.userId)
            put("pgStatus", listOf(PgStatus.CAPTURE_REQUEST, PgStatus.CAPTURE_RETRY, PgStatus.CAPTURE_SUCCESS).map { it. name })
